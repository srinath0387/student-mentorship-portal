...
// ── TEMPORARY: Data Recovery Route v2 (fixed) ────────────────────────────────
app.post('/admin/recover-from-backup', async (req: Request, res: Response) => {
  ...
  if (!recovery_host) {
    return res.status(400).json({ error: 'recovery_host is required' });
  }
  ...
  const summary: Record<string, number> = {
    ...
    student_passwords: 0, mentor_assignments: 0,
  };
  ...
  try {
    ...
    // KEY FIX: always take FIRST 5 of remaining missing — no offset needed
    // because each batch reduces allMissing for the next call automatically
    const allMissing = recStudents.rows.filter(
      (s: any) => !liveRolls.has((s.roll_number || '').toUpperCase())
    );
    const BATCH = 5;
    const batch = allMissing.slice(0, BATCH);
    const remaining = Math.max(0, allMissing.length - batch.length);

    console.log(`[Recovery v2] allMissing=${allMissing.length}, processing ${batch.length}, remaining after=${remaining}`);
    ...
    for (const s of batch) {
      try {
        ...
        for (const row of acad.rows) {
          await db.query(`INSERT INTO academics (student_id,semester,semester_gpa,backlogs,remarks,theory_grade) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING`,
            [row.student_id,row.semester,row.semester_gpa,row.backlogs,row.remarks,row.theory_grade]).catch(()=>{});
          summary.academics++;
        }
        const coding = await recoveryPool.query('SELECT * FROM coding_profiles WHERE student_id=$1', [roll]);
        for (const row of coding.rows) {
          await db.query(`INSERT INTO coding_profiles (student_id,platform,handle,score_rating,easy_count,medium_count,hard_count,repositories_count,contributions_count) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (student_id,platform) DO NOTHING`,
            [row.student_id,row.platform,row.handle,row.score_rating,row.easy_count,row.medium_count,row.hard_count,row.repositories_count,row.contributions_count]).catch(()=>{});
        ...
        for (const row of certs.rows) {
          await db.query(`INSERT INTO certifications (id,student_id,name,issuer,issue_date,expiry_date,credential_url,document_url) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (id) DO NOTHING`,
            [row.id,row.student_id,row.name,row.issuer,row.issue_date,row.expiry_date,row.credential_url,row.document_url]).catch(()=>{});
          summary.certifications++;
        }
        const tech = await recoveryPool.query('SELECT * FROM tech_skills WHERE student_id=$1', [roll]);
        for (const row of tech.rows) {
          await db.query(`INSERT INTO tech_skills (id,student_id,skill,level) VALUES ($1,$2,$3,$4) ON CONFLICT (id) DO NOTHING`,
            [row.id,row.student_id,row.skill,row.level]).catch(()=>{});
          summary.tech_skills++;
        }
        const soft = await recoveryPool.query('SELECT * FROM soft_skills WHERE student_id=$1', [roll]);
        for (const row of soft.rows) {
          await db.query(`INSERT INTO soft_skills (id,student_id,skill,rating) VALUES ($1,$2,$3,$4) ON CONFLICT (id) DO NOTHING`,
            [row.id,row.student_id,row.skill,row.rating]).catch(()=>{});
          summary.soft_skills++;
        }
        const ach = await recoveryPool.query('SELECT * FROM achievements WHERE student_id=$1', [roll]);
        for (const row of ach.rows) {
          await db.query(`INSERT INTO achievements (id,student_id,title,description,date) VALUES ($1,$2,$3,$4,$5) ON CONFLICT (id) DO NOTHING`,
            [row.id,row.student_id,row.title,row.description,row.date]).catch(()=>{});
        ...
        for (const row of pwds.rows) {
          await db.query(`INSERT INTO student_passwords (roll_number,password,updated_at) VALUES ($1,$2,$3) ON CONFLICT (roll_number) DO NOTHING`,
            [row.roll_number,row.password,row.updated_at]).catch(()=>{});
        ...
        for (const row of mentor.rows) {
          await db.query(`INSERT INTO mentor_assignments (roll_number,faculty_id,assigned_at) VALUES ($1,$2,$3) ON CONFLICT (roll_number) DO NOTHING`,
            [row.roll_number,row.faculty_id,row.assigned_at]).catch(()=>{});
          summary.mentor_assignments++;
        }
      } catch (rowErr: any) {
        console.error(`[Recovery v2] Error on ${s.roll_number}:`, rowErr.message);
      }
    }
    ...
    res.json({
      success: true,
      message: remaining > 0
        ? `Batch done! ${summary.students} restored. ${remaining} still remaining — call again (same body, no offset needed).`
        : `ALL DONE! ${summary.students} restored. No more students missing.`,
      restored: summary,
      totalMissingBeforeBatch: allMissing.length,
...
// ── END TEMPORARY RECOVERY ROUTE v2 ──────────────────────────────────────────
